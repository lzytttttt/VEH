    ...
    return (
      <div className="flex flex-col h-full bg-win-gray" style={{ gap: '4px', padding: '4px' }}>
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          {mobileTab === 'list' && <div className="flex flex-col h-full gap-1">{docListPanel}</div>}
          {mobileTab === 'editor' && <div className="flex flex-col h-full gap-1">{editorPanel}</div>}
          {mobileTab === 'assistant' && <div className="h-full">{assistantPanel}</div>}
        </div>
      </div>
    );
  }
